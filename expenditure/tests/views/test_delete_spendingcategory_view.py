...
class DeleteSpendingCategoryViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.url_delete_category = reverse('delete_spending_category', kwargs={'pk': self.category.pk})

    def test_delete_category_url(self):
        self.assertEqual(self.url_list_spending,'/spending/')
        self.assertEqual(self.url_delete_category, f"/spending-category/{self.category.id}/delete")
    ...
    def test_category_deleted(self):
        ...
        spending_category_exists = SpendingCategory.objects.filter(id=self.category.pk).exists()
        spending_category_limit_exists = Limit.objects.filter(id=self.category.limit.pk).exists()
