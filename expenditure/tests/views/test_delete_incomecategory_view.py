...
from expenditure.models import Transaction, IncomeCategory, Limit, User
...
class DeleteIncomeCategoryViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.url_delete_category = reverse('delete_income_category', kwargs={'pk': self.category.pk})

    def test_delete_category_url(self):
        self.assertEqual(self.url_list_incomings,'/incomings/')
        self.assertEqual(self.url_delete_category, f"/income-category/{self.category.id}/delete")
    ...
    def test_category_deleted(self):
        ...
        income_category_exists = IncomeCategory.objects.filter(id=self.category.pk).exists()
