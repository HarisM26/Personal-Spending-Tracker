...
class EditAndDeleteTransactionsViews(TestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('delete_spending', kwargs={
                           'id': self.transaction.pk})
        self.url_2 = reverse('delete_income', kwargs={
                             'id': self.transaction_incoming.pk})
    ...
    def test_edit_spending_details(self):
        ...
        self.assertIn('edit_spending_transaction.html',
                      (t.name for t in response.templates))

    def test_edit_incoming_details(self):
        self.client.login(email='johndoe@example.com', password='Password123')
        response = self.client.get(
            f'/incomings/edit/{self.transaction_incoming.pk}/')
        self.assertEqual(response.status_code, 200)
        self.assertIn('edit_income_transaction.html',
                      (t.name for t in response.templates))

    def test_successful_edit_spending_details(self):
        self.client.login(email='johndoe@example.com', password='Password123')
        response = self.client.post(
            f'/spending/edit/{self.transaction.pk}/', self.transaction_input)
        ...
        self.assertEqual(transaction.spending_category.pk,
                         self.transaction_input['spending_category'])
    ...
    def test_successful_edit_incoming_details(self):
        self.client.login(email='johndoe@example.com', password='Password123')
        response = self.client.post(
            f'/incomings/edit/{self.transaction_incoming.pk}/', self.incoming_transaction_input)
        transaction = IncomeTransaction.objects.get(
            pk=self.transaction_incoming.pk)
        ...
        self.assertEqual(transaction.income_category.pk,
                         self.incoming_transaction_input['income_category'])
        self.assertEqual(transaction.amount, Decimal('60.00'))
        self.assertEqual(response.status_code, 302)

    # def test_successful_receipt_edit(self):
    #     self.client.login(email='johndoe@example.com', password='Password123')
    ...
    def test_spending_limit_amount_change(self):
        ...
        transaction = SpendingTransaction.objects.get(pk=self.transaction.pk)
        self.assertEqual(
            transaction.spending_category.limit.remaining_amount, Decimal('20.00'))
    ...
    def test_delete_incoming_url(self):
        self.assertEqual(
            self.url_2, f'/incomings/delete/{self.transaction_incoming.pk}/')
